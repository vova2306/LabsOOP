class Iterator:
    def __init__(self, mn):
        self._key_cursor=0
        self._mn=mn
        self._keys=sorted(mn.keys())

    def __next__(self):
        try:
            num=self._keys[self._key_cursor]
            temp=self._mn[num]
            self._key_cursor+=1
            return num, temp
        except IndexError:
...
def dr_mn(e):
    it=Iterable(e.polynom)
    k=['{}x^{}'.format(temp, num) for num, temp in it]
    print('+'.join(k))
...
def m_l(e):
    ...
    for key, temp in it:
        ...
        if temp<min_n:
            min_n=temp
    print('Мін: {},'.format(min_n), 'Макс: {}'.format(max_n))
...
if __name__=='__main__':
    ...
    dr_mn(m1)
    dr_mn(m2)
    print('Найбільший та найменший коефіцієнти многочленів:')
    m_l(m1)
    m_l(m2)
    print('Найбільше відношення коефіцієнту многочлена до степеня, що йому відповідає:')
    m_g(m1)
    m_g(m2)
